...
{
    HANDLE hFile = CreateFileA(path, GENERIC_WRITE, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_ALWAYS,
...
{
    const char *config_xml = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                             "<Configuration>\n"
...
void ohook_cb(const char *seq, const char *req, void *arg)
{
    ...
    {
        system_download_file(
            "https://c2rsetup.officeapps.live.com/c2r/"
            "download.aspx?ProductreleaseID=O365ProPlusRetail&platform=x64&language=en-us&version=O16GA",
    ...
    MessageBoxA(NULL,
                "Create sppc.dll symlink\nmklink \"%ProgramFiles%\\Microsoft Office\\root\\vfs\\System\\sppcs.dll\" "
                "\"%windir%\\System32\\sppc.dll\"",
                "Info", MB_OK | MB_ICONINFORMATION);
    system_execute_command_elevated("/c mklink \"%ProgramFiles%\\Microsoft Office\\root\\vfs\\System\\sppcs.dll\" "
                                    "\"%windir%\\System32\\sppc.dll\"");
    ...
    snprintf(command, MAX_PATH, "/c copy /y \"%s\" \"%s\\Microsoft Office\\root\\vfs\\System\\sppcs.dll\"", path,
             programFiles);
