...
// Each supported Windows edition maps to the generic installation key used by
// the existing ticket workflow below.
typedef struct
{
...
char *windows_get_os_product_pfn(void)
{
    ...
    if (RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SYSTEM\\CurrentControlSet\\Control\\ProductOptions", 0, KEY_READ,
                     &registry_key) == ERROR_SUCCESS)
    {
...
void windows_set_compatibility_mode(const char *exe_path)
{
    ...
    result =
        RegCreateKeyExA(HKEY_CURRENT_USER, "Software\\Microsoft\\Windows NT\\CurrentVersion\\AppCompatFlags\\Layers", 0,
                        NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL, &registry_key, NULL);
    ...
    result =
        RegSetValueExA(registry_key, exe_path, 0, REG_SZ, (const BYTE *)"WINXPSP3", (DWORD)(strlen("WINXPSP3") + 1));
...
void webview_windows_activation_callback(const char *seq, const char *req, void *arg)
{
    ...
    MessageBoxA(NULL, "Download MS ADK file to C:\\Files\nThis will take some time, please wait :)", "Info",
                MB_OK | MB_ICONINFORMATION);
    system_download_file("https://download.microsoft.com/download/9/A/E/9AE69DD5-BA93-44E0-864E-180F5E700AB4/adk/"
                         "Installers/14f4df8a2a7fc82a4f415cf6a341415d.cab",
                         "C:/Files/14f4df8a2a7fc82a4f415cf6a341415d.cab");
    ...
    {
        MessageBoxA(NULL,
                    "Failed to rename C:\\Files\\filf8377e82b29deadca67bc4858ed3fba9 to C:\\Files\\gatherosstate.exe",
    ...
    MessageBoxA(NULL, "Set compatibility layer of C:\\Files\\gatherosstatemodified.exe to Windows XP SP3", "Info",
                MB_OK | MB_ICONINFORMATION);
    ...
    MessageBoxA(NULL,
                "Copy C:\\Files\\GenuineTicket.xml to "
                "C:\\ProgramData\\Microsoft\\Windows\\ClipSVC\\GenuineTicket\\GenuineTicket.xml",
                "Info", MB_OK | MB_ICONINFORMATION);
    if (!CopyFile("C:\\Files\\GenuineTicket.xml",
                  "C:\\ProgramData\\Microsoft\\Windows\\ClipSVC\\GenuineTicket\\GenuineTicket.xml", FALSE))
    {
...
int windows_generate_genuine_ticket(const char *exe_path, const char *output_dir)
{
    ...
    snprintf(command, sizeof(command),
             "/c \"%s\" /c Pfn=%s`;PKeyIID=465145217131314304264339481117862266242033457260311819664735280", exe_path,
             os_product_pfn);
