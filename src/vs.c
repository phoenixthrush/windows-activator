...
// Keep product names and their matching keys together so adding a supported
// edition is a one-line change.
...
static const size_t visual_studio_license_keys_count =
    sizeof(visual_studio_license_keys) / sizeof(visual_studio_license_keys[0]);

static int visual_studio_find_edition_index(const char *edition)
{
...
void webview_visual_studio_activation_callback(const char *seq, const char *req, void *arg)
{
    ...
    if (OpenClipboard(NULL))
    {
        ...
        snprintf(install_command, sizeof(install_command),
                 "/c C:/Windows/Temp/vs_installer.exe --add-product --product-key %s --passive",
                 visual_studio_license_keys[edition_index].license_key);
